...
def evaluate_model(tagged_sentences, model, word_to_ix, use_cuda):
    ...
    with open('LSTM_overall_performance_metrics.json', 'w') as f:
        json.dump(overall_metrics, f, indent=4)
    logger.info("Overall performance metrics saved to 'LSTM_overall_performance_metrics.json'.")
    ...
    with open('LSTM_per_pos_performance_metrics.json', 'w') as f:
        json.dump(per_tag_metrics, f, indent=4)
    logger.info("Per-POS performance metrics saved to 'LSTM_per_pos_performance_metrics.json'.")

    # Save confusion matrix
    np.save('LSTM_confusion_matrix.npy', conf_matrix)
    logger.info("Confusion matrix saved to 'LSTM_confusion_matrix.npy'.")
    ...
    plt.tight_layout()
    plt.savefig('LSTM_confusion_matrix.png')  # Save plot to file
    plt.show()
    logger.info("Confusion matrix plot saved as 'LSTM_confusion_matrix.png'.")
    ...
    with open('LSTM_most_mismatched_tags.json', 'w') as f:
        json.dump(most_mismatched_tags, f, indent=4)
    logger.info("Most mismatched tags saved to 'LSTM_most_mismatched_tags.json'.")
