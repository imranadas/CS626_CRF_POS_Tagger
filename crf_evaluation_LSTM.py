...
from crf_training_LSTM import CRFModel, Config

# Set up logging
logger = logging.getLogger()
logger.setLevel(logging.INFO)
file_handler = logging.FileHandler('Logs/crf_pos_LSTM_Evaluation_Log.log')
...
def evaluate_model(tagged_sentences, model, word_to_ix, tag_to_ix, use_cuda):
    ...
    with open('Model_Metrics/LSTM_overall_performance_metrics.json', 'w') as f:
        json.dump(overall_metrics, f, indent=4)
    logger.info("Overall performance metrics saved to 'Model_Metrics/LSTM_overall_performance_metrics.json'.")
    ...
    with open('Model_Metrics/LSTM_per_pos_performance_metrics.json', 'w') as f:
        json.dump(per_tag_metrics, f, indent=4)
    logger.info("Per-POS performance metrics saved to 'Model_Metrics/LSTM_per_pos_performance_metrics.json'.")

    # Save confusion matrix
    np.save('Model_Metrics/LSTM_confusion_matrix.npy', conf_matrix)
    logger.info("Confusion matrix saved to 'Model_Metrics/LSTM_confusion_matrix.npy'.")
    ...
    plt.tight_layout()
    plt.savefig('Model_Metrics/LSTM_confusion_matrix.png')
    plt.show()
    logger.info("Confusion matrix plot saved as 'Model_Metrics/LSTM_confusion_matrix.png'.")
    ...
    with open('Model_Metrics/LSTM_most_mismatched_tags.json', 'w') as f:
        json.dump(most_mismatched_tags, f, indent=4)
    logger.info("Most mismatched tags saved to 'Model_Metrics/LSTM_most_mismatched_tags.json'.")
